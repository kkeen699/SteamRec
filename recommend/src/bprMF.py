...
class bprMF(object):
    ...
    def config_global(self, MODEL_CLASS, HIDDEN_DIM, LAMBDA, LEARNING_RATE, BATCH_SIZE, target_stage_id, testing_stage_id):        
        ...
        self.testing_stage_id = testing_stage_id
    ...
    def evaluation(self, data_test, user_item_map, topK):
        ...
        target_stage_id = self.target_stage_id
        testing_stage_id = self.testing_stage_id
        ...
        with tf.Graph().as_default(), tf.Session() as session:
            ...
            saver = tf.train.Saver()
            saver.restore(session, MODEL_DIR + self.MODEL_NAME + ".model.ckpt")
            index_list = (data_test['max_stage_vali']>=testing_stage_id) + (data_test['max_stage_test']>=testing_stage_id)
            data_test_slice = data_test[index_list].values
            metric_vali, metric_test = self.evaluate_model_slice(u, s, testing_stage_id, data_test_slice, user_item_map, topK=topK)

...
class chainRec(object):
    ...
    def recommend(self, user_item_map, topK):
        n_user = self.n_user
        n_item = self.n_item
        n_stage = self.n_stage
        ...
        item_genre_map = {}
        with open(DATA_DIR + 'steam_item_genre_map', 'r') as f:
            for row in f:
                line = eval(row)
                item_genre_map[line['item_id']] = line['genre']
        
        genre_item_map = {}
        for item in item_genre_map:
            for genre in item_genre_map[item]:
                if genre in genre_item_map:
                    genre_item_map[genre].append(item)
                else:
                    genre_item_map[genre] = [item]
        
        item_name_map = {}
        with open(DATA_DIR + 'steam_item_name.csv', newline='', encoding="utf-8") as csvfile:
            spamreader = csv.reader(csvfile, delimiter=",", quotechar='"')
            for row in spamreader:
                item_name_map[int(row[0])] = row[1]

        genre_name_map = {}
        with open(DATA_DIR + 'steam_genre_map') as f:
            for row in f:
                line = eval(row)
                for name in line.keys():
                    genre_name_map[line[name]] = name
        ...
        with tf.Graph().as_default(), tf.Session() as session:
            ...
            print("recommend by model: " + MODEL_NAME + " ...")
            
            user_list = np.random.choice(n_user, 5).ravel()

            _s_list = np.array(s.eval(feed_dict={u:user_list}))
            for i, _u in enumerate(user_list):
                item_u = user_item_map[_u]
                item_list = item_u[:,0]                  
                _s = _s_list[i][:, target_stage_id]
                _s[np.array(item_list)] = -10000
                rec = _s.argsort()[::-1]

                print("user: ", _u)
                print("\nChainRec: ")
                print(", ".join([item_name_map[i] for i in rec[:topK]]))
                genre_pre = self.genre_analysis(user_item_map[_u], item_genre_map)

                for g in genre_pre:
                    rec_g = []
                    for i in rec[topK:]:
                        if i in genre_item_map[g]:
                            rec_g.append(i)
                        if len(rec_g) == 10:
                            break

                    print("\ngrnre: ", genre_name_map[g])
                    print(", ".join([item_name_map[i] for i in rec_g]))
                print("======================================================================")

    def genre_analysis(self, item_list, item_genre_map):
        preference = {}

        for i, score in item_list:
            genre = item_genre_map[i]
            for g in genre:
                if g in preference:
                    preference[g] += score
                else: 
        ...
        pre_sort = sorted(list(preference.items()), key=lambda x: x[1], reverse=True)
        
        pre = []
        for i in range(5):
            pre.append(pre_sort[i][0])
        return pre

...
def train(DATA_NAME, n_stage, method, embed_size, lbda, target_stage, testing_stage):
    ...
    if method == "chainRec_uniform":
        ...
        myModel.config_global(MODEL_CLASS="bprMF", HIDDEN_DIM=embed_size, 
                              LAMBDA=lbda, LEARNING_RATE=0.001, BATCH_SIZE=256,
                              target_stage_id=target_stage, testing_stage_id=testing_stage)
        myModel.load_samples(training_samples, validation_samples)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='steam', 
                        help="specify a dataset from [steam]")
    ...
    parser.add_argument('--target_stage_id', default=3,
                        help="the target stage", type=int)
    parser.add_argument('--testing_stage_id', default=3,
                        help="the target stage", type=int)
    args = parser.parse_args()
    train(args.dataset, args.nStage, args.method, args.embedSize, args.l2, args.target_stage_id, args.testing_stage_id)
